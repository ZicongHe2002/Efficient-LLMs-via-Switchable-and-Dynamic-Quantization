...
a_bits = 32
w_bits = [8,16,32]
loraPath = "main2_epoch_6_valid_avg_6.6855_lora_weights.bin"
exp_name = "main2"
#  新加的 step 5
current_w_bits = 32
lora_quant_configs = {
...
def train_loop(dataloader, model, loss_fn, optimizer, lr_scheduler, epoch, total_loss):
    ...
    new_lora_quant_configs = {
        EMBEDDING_KEY: {"r": 8, "w_bits": 32, "a_bits": 32},
        ATTN_KEY: {
            "layer_0_3": {"r":8, "w_bits": 32, "a_bits": 32, "lora_dropout": 0.1},
            "layer_4_7": {"r": 8, "w_bits": 32, "a_bits": 32, "lora_dropout": 0.1},
            "layer_8_11": {"r": 8, "w_bits": 32, "a_bits": 32, "lora_dropout": 0.1}
        },
        MLP_KEY: {
            "layer_0_3": {"r": 8, "w_bits": 32, "a_bits": 32, "lora_dropout": 0.1},
            "layer_4_7": {"r": 8, "w_bits": 32, "a_bits": 32, "lora_dropout": 0.1},
    ...
    modifyValuesWithConfig(model, new_lora_quant_configs)
    train_last_loss = inf
    for batch, (X, start_pos, end_pos) in enumerate(dataloader, start=1):
        ...
        train_last_loss = total_loss / (finish_batch_num + batch)
        progress_bar.set_description(f'loss: {train_last_loss:>7f}')
        progress_bar.update(1)
    return total_loss, train_last_loss
...
def test_loop(dataloader, dataset, model):
    ...
    print(f"F1: {result['f1']:>0.2f} EM: {result['em']:>0.2f} AVG: {result['avg']:>0.2f} BLEU: {result['bleu']:>0.2f}\n")
    return result
...
if __name__ == '__main__':
   ...
   # model = addLoraQuantizer(model, config, r=r, w_bits=w_bits, a_bits=a_bits).to(device)
    ...
    for t in range(epoch_num): #想办法保存epoch的次数，然后这里从epoch的次数开始训练，
        ...
        total_loss, last_loss = train_loop(train_dataloader, model, loss_fn, optimizer, lr_scheduler, t + 1, total_loss)
        ...
        #     best_avg_score = avg_score
            ...
            if len(lora_dict) == 0:
                torch.save(model.state_dict(), f'{exp_name}_epoch_{t + 1}_valid_avg_{avg_score:0.4f}_model_weights.bin')
            else:
                torch.save(lora_dict, f'{exp_name}_epoch_{t + 1}_valid_avg_{avg_score:0.4f}_lora_weights.bin')
    print("Done!")