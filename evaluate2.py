...
def calc_bleu_score(answers, prediction):
    bleu_scores = []
    for ans in answers:
        ans_tokens = mixed_segmentation(ans, rm_punc=True)
        prediction_tokens = mixed_segmentation(prediction, rm_punc=True)
        bleu_scores.append(sentence_bleu([ans_tokens], prediction_tokens))
...
def evaluate(predictions, references):
    ...
    for query_id, answers in ref.items():
        ...
        bleu = calc_bleu_score(answers,prediction)
    f1_score = 100.0 * f1 / total_count
    em_score = 100.0 * em / total_count
    bleu_score = 100.0 * bleu / total_count
    return {
